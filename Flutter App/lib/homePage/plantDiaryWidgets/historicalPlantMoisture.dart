...
import 'package:best_flutter_ui_templates/mqttClientWrapper.dart';
import 'package:mqtt_client/mqtt_client.dart';
import '../homePage_theme.dart';

class HistoricalPlantMoistureView extends StatefulWidget {
  final AnimationController animationController;
  final Animation animation;
  final MQTTClientWrapper client;

  const HistoricalPlantMoistureView(
      {Key key, this.animationController, this.animation, this.client})
      : super(key: key);

  @override
  _HistoricalPlantMoistureView createState() =>
      _HistoricalPlantMoistureView(animationController, animation, client);
}

class _HistoricalPlantMoistureView extends State<HistoricalPlantMoistureView> {
  Uint8List base64Image = base64Decode(
  ...
  _HistoricalPlantMoistureView(AnimationController animationControlleraa,
      Animation animationaa, MQTTClientWrapper clienta) {
  ...
  Widget build(BuildContext context) {
    client.updates.listen((List<MqttReceivedMessage<MqttMessage>> c) {
      c.forEach((element) {
        if (element.topic == "plantMoisturesChart") {
          final MqttPublishMessage recMess = element.payload;
          var object = base64Decode(MqttPublishPayload.bytesToStringAsString(
              recMess.payload.message));
          setState(() {
            // object==null? base64Image = "" : base64Image==object;
            base64Image = object;
          });
        }
      });
